...
public class BlogController {
    ...
    public String blogs(@PageableDefault(size = 10, sort = {"updateTime"}, direction = Sort.Direction.DESC) Pageable pageable, BlogQuery blogQuery, Model model) {
        model.addAttribute("types", typeService.listType()); // 在model中加入所有的type，用于搜索栏的下拉框
        model.addAttribute("page", blogService.listBlog(pageable, blogQuery));
        return LIST;
    }

    // 使用搜索功能时，动态刷新页面中的一部分，而不刷新整个页面
    ...
    public String search(@PageableDefault(size = 10, sort = {"updateTime"}, direction = Sort.Direction.DESC) Pageable pageable, BlogQuery blogQuery, Model model) {
        model.addAttribute("page", blogService.listBlog(pageable, blogQuery));
        return "/admin/blogs :: blogList"; // 返回一个blogList片段，定义在blogs.html中
    }

    private void setTypeAndTag(Model model) {
        model.addAttribute("types", typeService.listType()); // 在model中加入所有的type，用于选择博客的分类
        model.addAttribute("tags", tagService.listTag()); // 在model中加入所有的tag，用于为博客添加标签
    }
    ...
    public String edit(@PathVariable Long id, Model model) {
        ...
        blog.init(); // 为了在页面中使用tagIds，调用init方法
